...
tsdify = function(DATA, reverse=F){
  ...
  if(reverse){
    ...
    tmp = tmp %>%
      mutate(Project_Wave = as.numeric(Project_Wave)) %>%
      mutate(Project_Wave = if_else(CrossProject_ID %in% NCPs$CrossProject_ID, Project_Wave+1, Project_Wave))
    ...
    if(grepl("NCP_Group",names(DATA)) %>% any){
      ...
      tmp$Project_Wave = ifelse(tmp$NCP_Group %in% "Start rest",
                                tmp$Project_Wave-1,
                                tmp$Project_Wave)
    ...
    if(grepl(";", DATA$Site_Number) %>% any){
      ...
      for(i in 1:length(Folder)){
        Folder[i] = ifelse(!is.na(DATA$Site_Number[i]),
                           paste(Folder[i], strsplit(DATA$Site_Number[i],";")[[1]], collapse=";", sep="_"),
                           NA)
      }
    }else{
