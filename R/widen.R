...
widen = function(data, by, ColumnList){
  ...
  if(SEP=="skip"){
    ...
    # IndexColumn = grep(paste0(by,"$"), names(DATA2))
    #
    DATA2 = DATA2 %>%
      gather(variable, val, -(1:4), na.rm=T)  %>%
      distinct() %>% arrange_("CrossProject_ID",by)

    DATA2[,4] = paste0(SEP,DATA2[,4] %>% unlist)

    DATA4 = DATA2 %>% arrange(variable) %>%
      unite(temp, c(4, variable) ) %>% distinct()

    # NBM w4 has spread in weeks/months between Curato and Oslo.Prisma. coerce these into mean age
    tmp = DATA4 %>% filter(CrossProject_ID > 9000000 & temp %in% "W4_Age") %>%
    ...
    DATA4 = DATA4 %>%
      ...
      rbind.data.frame(tmp) %>% na.omit() %>% distinct
    ...
    DATA4 = DATA2 %>%
      gather(variable, val, -(1:IndexColumn), na.rm=T) %>%
      na.omit() %>%
      distinct() %>%
      arrange(variable)
    ...
    DATA3 = DATA3 %>%
      ...
      mutate(N_Scans=sum(N_Scans)) %>% as.data.frame() %>%
