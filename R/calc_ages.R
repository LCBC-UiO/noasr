...
calc_ages = function(data) {
  ...
  data = data %>% 
    mutate(Date = ifelse(!is.na(MRI_Date), MRI_Date, Test_Date) %>% as.Date(origin = "1970-01-01"))
  ...
  data = data %>% 
    mutate(MRI_Age = ifelse(!is.na(MRI_Date), difftime(MRI_Date, Birth_Date, units = "days")/365.25, NA),
    ...
    mutate(Age = ifelse(is.na(Age), (Date - Birth_Date)/365.25,Age))
