...
#'
#' @import tidyverse
#' @importFrom stats ave filter lag na.omit time
#' @export
...
calc_ages = function(data) {
    ...
    DATES = data %>% group_by(Project_Name, Project_Wave) %>% summarise(Mean = mean.Date(Date, na.rm = T), Min = min(Date, 
    ...
    data = data %>% filter(!is.na(CrossProject_ID)) %>% arrange(CrossProject_ID, Date)
    ...
    tmp = data %>% select(CrossProject_ID, Project_Number, Project_Wave) %>% unique %>% group_by(CrossProject_ID) %>% mutate(Subject_Timepoint = ave(CrossProject_ID, 
        CrossProject_ID, FUN = seq_along))
    
    data = data %>% select(-one_of("Subject_Timepoint")) %>% left_join(tmp, by = c("CrossProject_ID", "Project_Number", "Project_Wave")) %>% 
        group_by(CrossProject_ID) %>% mutate(Interval_MRI_Test = ifelse(!is.na(Test_Date) & !is.na(MRI_Date), difftime(Test_Date, 
        MRI_Date, units = "days"), NA), Interval_LastVisit = lag(Age, 1), Interval_FirstVisit = first(Age)) %>% mutate(Interval_LastVisit = Age - 
        Interval_LastVisit, Interval_FirstVisit = Age - Interval_FirstVisit) %>% mutate(Interval_LastVisit = ifelse(Interval_LastVisit == 
        0, lag(Interval_LastVisit, 1), Interval_LastVisit))
    
    data = data %>% arrange(CrossProject_ID, Subject_Timepoint) %>% select(-Date) %>% as.data.frame()
